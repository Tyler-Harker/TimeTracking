...
export default function DashboardPage() {
  ...
  // Be defensive: a fresh org with no time entries / invoices can produce a payload
  // where summed numeric fields come back as null from the API, which would crash
  // .toFixed/.toString. The handler does coalesce to 0, but we coerce again here so
  // a shape regression on the API side doesn't take the whole page down.
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <StatCard
          label="Total Clients"
          value={num(data.clientCount).toString()}
          icon={<svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg>}
          iconColor="#3B82F6"
        />
        <StatCard
          label="Active Projects"
          value={`${num(data.activeProjectCount)}/${num(data.projectCount)}`}
        ...
        <StatCard
          ...
          value={num(data.totalHoursYtd).toFixed(1)}
        ...
        <StatCard
          ...
          value={`$${num(data.totalInvoicedYtd).toFixed(0)}`}
        ...
        <StatCard
          ...
          value={num(data.totalUninvoicedHours).toFixed(1)}
          icon={<svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
          iconColor="#94A3B8"
        />
        <StatCard
          label="Uninvoiced Amount"
          value={`$${num(data.totalUninvoicedAmount).toFixed(0)}`}
