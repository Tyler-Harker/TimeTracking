...
export default function ProjectTimeEntryNewPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setSaving(true);
    try {
      await timeEntryRepository.create({ projectId: params.id, date, hours: parseFloat(hours), description: description || undefined, isBillable });
      router.push("/time-entries");
    } catch (e) {
      setError(e instanceof Error ? e.message : "Failed to create time entry");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="p-6 max-w-2xl">
      <h1 className="text-2xl font-bold text-slate-50 mb-6">Log Time</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <label className="flex items-center gap-2 text-sm text-slate-300">
          <input type="checkbox" checked={isBillable} onChange={(e) => setIsBillable(e.target.checked)} className="rounded border-slate-600 bg-slate-800 text-indigo-500 focus:ring-indigo-500" />
          Billable
        </label>
